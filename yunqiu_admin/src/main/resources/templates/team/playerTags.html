...
<script>
    $(document).ready(function(){
        ...
        $("#list4").jqGrid({
            ...
            shrinkToFit:true,
            colModel:[
                {label:"标签Id",name:'playertagsId',index:'playertagsId', width:'15%',align:'center',hidden: true},
                {label:"球员标签",name:'tagsName',index:'tagsName', width:'35%',align:'center'},
            ...
            jsonReader:{
                id: "playertagsId",//设置返回参数中，表格ID的名字为blackId
        ...
        $("#btn_add").bind("click",function(){
            ...
            $("#add").bind("click",function(){
                ...
                $("#addForm").ajaxSubmit({
                    ...
                    url: "/playertags/addPlayerTags",//请求地址
        ...
        $("#btn_edit").bind("click",function(){
            ...
            if (selectedId == null){
                ...
                $("#up_tagsName").val(select.tagsName);
                $("#up_playertagsId").val(select.playertagsId);

                $("#update").bind("click",function(){
                    tagsName = $("#up_tagsName").val();

                    if(!tagsName.length>0){
                        $("#vu_tagsName").html("请输入标签名称");
                        return;
                    }
                    ...
                    $("#updateForm").ajaxSubmit({
                        type: "POST",//提交类型
                        dataType: "json",//返回结果格式
                        url: "/playertags/upPlayerTags",//请求地址
        ...
        $("#btn_delete").bind("click",function(){
            ...
            var select= jQuery("#list4").jqGrid('getRowData', selectedId);
            if (selectedId == null){
                swal("球员标签","请选择需要删除的信息！","info");
            }else{
                $("#delete_winfrom").modal('show');
                $("#delete").bind("click",function(){
                    $.ajax({
                        url:"/playertags/delPlayerTags",
                        data:{
                            playertagsId:select.playertagsId
                        },
                        success:function(data){
                            if (data["error_code"] == 1500){
                                swal("系统错误!", data["msg"], "error");
                ...
                <div class="ibox-title">
                    <form class="form-inline">
                        ...
                        <div class="form-group">
                            <label>标签名称</label>
                            <input type="text" id="s_tagsName" name="tagsName" class="form-control"/>
                        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="myModalLabel">球员标签添加</h2>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-sm-5 control-label">标签名称<span class="validt" id="va_tagsName"></span></label>
                        <input type="text" id="tagsName" name="tagsName" class="form-control"/>
                    </div>
...
<form id="updateForm">
    <div id="update_winfrom" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">球员标签修改</h2>
                    <input type="hidden" id="up_playertagsId" name="playertagsId"/>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="col-sm-5 control-label">标签名称<span class="validt" id="vu_tagsName"></span></label>
                        <input type="text" id="up_tagsName" name="tagsName" class="form-control" />
                    </div>
