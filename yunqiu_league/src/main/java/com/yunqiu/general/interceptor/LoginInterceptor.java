...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (Utils.isNull(access_token) || Utils.isNull(user_id)){
            response.getWriter().print(GsonUtil.toJson(ControllerReturnBase.errorResule(1501,"缺少用户id跟token"),false));
            return false;
        }else if (tokens.verifyToken(user_id,access_token)){
            response.getWriter().print(GsonUtil.toJson(ControllerReturnBase.errorResule(1504,"未登录或token已失效，请重新登录"),false));
            return false;
        }
        return true;
    }
